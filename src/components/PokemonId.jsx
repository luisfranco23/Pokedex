    ...
    <section className="poke-id">
      <aside className="poke-head">
        ...
        <div className="poke-type-ability">
          ...
          <p className="title__info purple">{`${pokemonId?.types[1]?.type?.name === undefined ? '-○-' : pokemonId?.types[1].type.name}`}</p>
        ...
        <div className="stats">
          ...
          <div className="head">
            ...
            <div className={`info__var end--${pokemonId?.stats[0].base_stat}`}></div>
          <div className="head">
          <p className="info_id">{pokemonId?.stats[1].stat.name}</p>
          <p>{pokemonId?.stats[1].base_stat} /150</p>
          </div>
            <div className={`info__var end--${pokemonId?.stats[1].base_stat}`}></div>
          <div className="head">
          <p className="info_id">{pokemonId?.stats[2].stat.name}</p>
          <p>{pokemonId?.stats[2].base_stat} /150</p>
          </div>
            <div className={`info__var end--${pokemonId?.stats[2].base_stat}`}></div>
          <div className="head">
            <p className="info_id">{pokemonId?.stats[5].stat.name}</p>
            <p>{pokemonId?.stats[5].base_stat}/150</p>
            </div>
              <div className={`info__var end--${pokemonId?.stats[5].base_stat}`}></div>
              {/* <div className={`info_progres end--${pokemonId?.stats[5].base_stat}`}></div> */}
        </div>
      </aside>
        ...
        <div className="info">
            {
              pokemonId?.moves.map(e => (
                <button key={e.move.url} className="btn-info">{e.move.name}</button>
              ))
            }
        </div>
      </aside>
